...
public enum MotionCalculate {
    ...
    public void collisionDetectWithField(float roundTime) {
        for (int i = 0; i < snapShot.ballList.size(); i++) {
            ...
            ball.collisionFlagsWithField = collisionDetectWithFieldForBall(ball, roundTime);
        }
    ...
    private void updateParametersOfBall(float roundTime) {
        ...
        for (Ball ballA : snapShot.ballList) {
            ...
            ArrayList<Boolean> collideFlags = collisionDetectWithFieldForBall(ballA, roundTime);
            for (Boolean flag : collideFlags)
            {
                if (flag == true)
                    return;
            }
    ...
    public ArrayList<Boolean> collisionDetectWithFieldForBall(Ball ball, float roundTime) {

        ArrayList<Boolean> collideFlags = new ArrayList<>();
        ...
        if (newYLeftTop <= 0 && (newXLeftTop > 0 || newXLeftTop >= newYLeftTop) &&
            ...
            collideFlags.add(true);
        else
            collideFlags.add(false);
